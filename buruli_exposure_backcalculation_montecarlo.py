...
# Use ONE RNG everywhere (NumPy + SciPy). We also seed Python's `random` and legacy NumPy
# in case any dependency calls them internally.
if RANDOM_SEED is not None:
    random.seed(RANDOM_SEED)
    np.random.seed(RANDOM_SEED)

# Single source of randomness for this whole script:
...
def gamma_dist_bootstrap(data, force_loc=True, rng=None):
    """Bootstrap Gamma fit by resampling and refitting (RNG-controlled)."""
    if rng is None:
        rng = np.random.default_rng()

    idx = rng.integers(0, len(data), size=len(data), endpoint=False)
    random_sample = np.asarray(data)[idx]
...
# To ensure deterministic parallel bootstrapping, pre-generate per-iteration seeds
# from the single master RNG.
N_BOOT = 10000
boot_seeds = rng.integers(0, 2**32 - 1, size=N_BOOT, dtype=np.uint32)

results = Parallel(n_jobs=10, verbose=0)(
    delayed(gamma_dist_bootstrap)(samples, rng=np.random.default_rng(int(s)))
    for s in tqdm(boot_seeds, disable=True)
)
...
for row in range(table_S2.shape[0]):
    ...
    # Use the master RNG here as well (SciPy supports numpy Generator as random_state)
    incubation_periods = gamma.rvs(shape, scale=scale, size=200, random_state=rng)
